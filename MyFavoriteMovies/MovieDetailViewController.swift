...
class MovieDetailViewController: UIViewController {
    ...
    @IBAction func toggleFavorite(sender: AnyObject) {
        ...
        /*
         * if already favorited then unfavorite from favorite list
        ...
        */
        ...
        // build the http body. Used in POST requests
        ...
        let task = appDelegate.sharedSession.dataTaskWithRequest(request) { (data, response, error) in
            ...
            guard let statusCode = (response as? NSHTTPURLResponse)?.statusCode where statusCode >= 200 && statusCode <= 299 else {
                ...
                print(" toggleFavorite: Invalid Response Code ")
                return
            }
            ...
            // Do we have any data?
            guard ((data) != nil) else {
                print("toggleFavorite: No Request Data")
                return
            }
            ...
            do {
                ...
                parsedResult = nil
                print("toggleFavorite : could not parse the data as JSON: \(data)")
                return
            }
            
            
            /* 6. Use the data */
            if  let status_code  = parsedResult[Constants.TMDBResponseKeys.StatusCode] as? Int {
                
                /* The status code depends on the status of
                * the variable 'shouldFavorite'
                * 1 or 12 -> favorite added
                * 13 -> removed from favorites
                */
                if shouldFavorite && ( ( status_code == 1) || (status_code == 12) )
                {
                    self.isFavorite = shouldFavorite
                } else if  !shouldFavorite && (  status_code == 13 )  {
                    self.isFavorite = shouldFavorite
                } else {
                    print("toggleFavorite : Invalid 'status_code' in \(parsedResult)")
                    return
                ...
                print("toggleFavorite : Cannot find key 'status_code' in \(parsedResult)")
                return
            }
        ...
        /* If the favorite/unfavorite request completes, 
         * then use this code to update the UI...
        */
